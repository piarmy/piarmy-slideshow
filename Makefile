...
# Best to leave as is
DOCKER_TASKNAME   = piarmy-slideshow
...
shell: stop
	...
	@echo ""
	@docker run -it --rm --privileged --entrypoint="" --name=$(DOCKER_TASKNAME) $(DOCKER_IMAGE_NAME) /bin/ash

run: stop
	@docker run -d --rm --privileged --name=$(DOCKER_TASKNAME) $(DOCKER_IMAGE_NAME)
